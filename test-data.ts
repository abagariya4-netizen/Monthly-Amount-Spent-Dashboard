...
async function run() {
  const periods = [
    { label: 'Apr', since: '2026-04-01', until: '2026-04-30' },
  ];
  ...
  for (const p of periods) {
    ...
    let top = 0, mid = 0, bot = 0, growth = 0;
    ...
    for (const row of data) {
      ...
      if (cat === 'Mat' || cat === 'Group') {
        const funnel = classifyFunnel(cName);
        if (funnel === 'Top') top += spend;
        if (funnel === 'Mid') mid += spend;
        if (funnel === 'Bot') bot += spend;
        if (funnel === 'Growth') growth += spend;
      }
    }
    
    console.log(`${p.label} - Top: ${top}, Mid: ${mid}, Bot: ${bot}, Growth: ${growth}, Total: ${top+mid+bot+growth}`);
    console.log(`Expected - Top: 835229, Mid: 12287914, Bot: 4793040, Growth: 1426296, Total: 19342479`);
  }
}
